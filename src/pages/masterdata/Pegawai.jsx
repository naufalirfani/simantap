...
import { fetchPegawai, fetchPetaJabatan, syncPegawai } from '../../services/apiService';
...
const Pegawai = () => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setRefreshKey((k) => k + 1);
      Swal.fire({ icon: 'success', title: 'Sukses', text: 'Sinkronisasi pegawai selesai', timer: 2000, showConfirmButton: false });
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Gagal', text: err.message || 'Sinkronisasi gagal' });
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 'w-48',
      render: (item) => (
        <span className="text-md text-gray-700 dark:text-gray-300">{item.nip}</span>
      ),
    },
    {
      key: 'unit_kerja',
      ...
      render: (item) => (
        <div>
          ...
          <div className="flex gap-2 mt-1">
            {item.jenis_jabatan && (
              <span className="inline-flex items-center px-2 py-0.5 rounded text-md font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                {item.jenis_jabatan}
              </span>
            )}
            {item.golongan && (
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mb-4 flex justify-end">
        <IconButton onClick={handleSync} variant="primary" size="lg" disabled={isSyncing} title="Sinkronisasi">
          {isSyncing ? <i className="fas fa-spinner fa-spin mr-2" /> : <i className="fas fa-sync mr-2" />}Sinkronisasi
        </IconButton>
      </div>
